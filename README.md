...
- 在 LCD 上播放 `idle`、`noticed`、`listening`、`thinking`、`happy`、`confused`、`sad`、`sleeping` 和 `angry` 表情。
...
- 支持全部 9 种表情和自然眨眼动画，开机即可使用，离线也能播放。
...
- 生成结果仍包含相同的 9 种表情，每种表情 5 帧；设备下载资源包后可在本地播放。
